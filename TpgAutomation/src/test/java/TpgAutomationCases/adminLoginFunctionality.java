...
public class adminLoginFunctionality extends newCheckout {
	...
	public void launchTPGAdminPortal() throws Throwable {
		try {
			...
			test = extent.createTest("TPG Admin Portal").pass(
					MarkupHelper.createLabel("TPG Admin Portal has been opened Successfully!.", ExtentColor.GREEN));
			test.pass(
					MarkupHelper.createLabel("Email and Password fields are available for login!.", ExtentColor.GREEN));

		} catch (Exception e) {
			System.out.println(e);
			test = extent.createTest("TPG Admin Portal").fail(
					MarkupHelper.createLabel("TPG Admin Portal has not been loaded successfully!.", ExtentColor.RED));
			extent.flush();
	...
	public void adminSignUpCredentials() throws Throwable {
		try {
			...
			test = extent.createTest("Enter Admin Credentials")
					.pass(MarkupHelper.createLabel("Email and Password has been entered", ExtentColor.GREEN));
		} catch (Exception e) {
			System.out.println(e);
			test = extent.createTest("TPG Admin Portal")
	...
	public void loginToAdminPortal() throws Throwable {
		try {
			...
			test = extent.createTest("Admin Login").pass(MarkupHelper
					.createLabel("Admin with Email " + adminUserEmail_ + "has been logged in!.", ExtentColor.GREEN));
		} catch (Exception e) {
			System.out.println(e);
			test = extent.createTest("TPG Admin Portal").fail(MarkupHelper.createLabel(
					"Admin has entered invalid credentails , So not been able to logged in!.", ExtentColor.RED));
			extent.flush();
